...
export function LandingHero() {
  return (
    <section className="flex w-full flex-col">
            ...
            {navItems.map((item) =>
              item.href ? (
                <Link
                  key={item.label}
                  href={item.href}
                  className={item.active ? "text-[var(--ink)]" : "hover:text-[var(--ink)]"}
                >
                  {item.label}
                </Link>
              ) : (
      ...
      <FrameShell className="border-t-0">
        <div className="relative isolate min-h-[480px] overflow-hidden bg-black sm:min-h-[560px] lg:min-h-[624px]">
          <img
            alt=""
            className="absolute inset-0 h-full w-full object-cover opacity-95"
            src={HERO_ART}
          />
          <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(0,0,0,0.92)_0%,rgba(0,0,0,0.72)_44%,rgba(0,0,0,0.2)_100%)]" />
